import Image from "next/image";
import { MapPin, FileText } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface AboutProps {
  personal: {
    name: string;
    title: string;
    bio: string;
    location: string;
    available: boolean;
    availableText: string;
    whatsapp: string;
    cvUrl: string;
    photo: string;
  };
}

export default function AboutSection({ personal }: AboutProps) {
  return (
    <section className="py-16 mt-4">
      <div className="flex flex-col md:flex-row items-center gap-12">
        {/* Photo */}
        <div className="flex-shrink-0">
          <div className="relative w-[280px] h-[340px] md:w-[350px] md:h-[400px] rounded-2xl overflow-hidden shadow-lg">
            <Image
              src={personal.photo}
              alt={personal.name}
              fill
              className="object-cover"
              priority
            />
          </div>
        </div>

        {/* Info */}
        <div className="flex-1">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-2 tracking-tight">
            Hi, I&apos;m {personal.name.split(" ")[0]} 👋
          </h1>
          <h4 className="text-lg font-semibold text-green-600 dark:text-green-400 mb-5">
            {personal.title}
          </h4>

          <p className="text-muted-foreground text-lg leading-relaxed mb-6 max-w-xl">
            {personal.bio}
          </p>

          <div className="mb-6 space-y-1.5">
            <div className="flex items-center gap-2 text-muted-foreground text-sm">
              <MapPin size={15} />
              <span>{personal.location}</span>
            </div>
            {personal.available && (
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-500 dot-pulse inline-block" />
                <span className="text-green-600 dark:text-green-400 text-sm font-bold">
                  {personal.availableText}
                </span>
              </div>
            )}
          </div>

          <div className="flex flex-wrap gap-3">
            <a
              href={personal.whatsapp}
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button className="rounded-full bg-green-500 hover:bg-green-600 text-white px-6 gap-2 text-base">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                Let&apos;s Talk
              </Button>
            </a>
            <a
              href={personal.cvUrl}
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button
                variant="default"
                className="rounded-full px-6 gap-2 text-base"
              >
                <FileText size={18} />
                Download CV
              </Button>
            </a>
          </div>
        </div>
      </div>
    </section>
  );
}
